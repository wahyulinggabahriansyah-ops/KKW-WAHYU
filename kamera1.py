...
try:
    with open('koordinat_parkir_1.json', 'r') as f:
        data_mapping = json.load(f)
except FileNotFoundError:
    print("Error: File koordinat_parkir_1.json tidak ditemukan. Jalankan mapping.py terlebih dahulu.")
    exit()
...
try:
    ...
    cursor = conn.cursor()
    cursor.execute("SELECT total_masuk FROM status_kamera WHERE id_kamera = 'Kamera_01'")
    row = cursor.fetchone()
...
nama_jendela = "Smart Parking (Kamera 01)"
    ...
    for i, area in enumerate(area_parkir_list):
        ...
        if tracker_terisi[i] >= batas_waktu_frame:
            ...
            if status_akhir[i] == "KOSONG":
                total_masuk += 1
                print(f"[Kamera 01] Petak P{i+1} TERISI! Total kendaraan parkir: {total_masuk}")
